...
class SeeCommentSharePostViewModel @Inject constructor(
    private val appPreferences: AppPreferences,
    private val localRepository: LocalRepository
) : BaseViewModel() {
    var isPost = MutableLiveData<Post>()
    var informationUser = MutableLiveData<User>()
    var informationUserShare = MutableLiveData<User>()
    ...
    fun getNotificationCommentById(senderId: Long, userId: Long, postId: Long, commentId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            val notification =
                localRepository.getNotificationCommentById(senderId, userId, postId, commentId)
            if (notification != null) {
                notificationCommentById.postValue(notification)
            }
        }
    }
    ...
    // lấy data user của bài post
    fun getInformationUserShare(id: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            val user = localRepository.getId(id)
            if (user != null) {
                informationUserShare.postValue(user)
            }
        }
    }
}